...
| **Per-dealer isolation** | Optional **`ENFORCE_STAFF_SINGLE_COMPANY=true`**: **`ADMIN`** users are **company-bound** like other roles (no cross-tenant `assertSameCompany` bypass; **`GET /companies`** returns only their dealer; CaRGOS/SDI submission lists require a company filter when they would otherwise scan all tenants). **`GET /auth/me`** includes **`adminCrossCompanyAccess`** for the desk. See [STRUCTURE.md](STRUCTURE.md) §7. |
